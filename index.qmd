...
- multiple losses: does the method use more than one loss for training?
...
- recurrent events: cna the method handle recurrent events?
- TVF: can the method handle time-varying features?
- TVE: can the method estimate time-varying effects?
- high dimensionality: is the method specifically designed for high-dimensional (e.g., omics) features?
...
- code: is the code for the method and experiments publicly available and, if so, where? (Further implementations of the method, if available, are shown by expanding the respective row.)
- datasets (non-private): which non-private (i.e., public or registered access) datasets is the method evaluated on in the respective paper?
- simulation: is the method additionally evaluated on synthetic data?
